...
import { DIALOGS_AND_MENUS, useDialogStore } from '@/stores/dialog';
...
const close = () => {
    if (props.dialogId === dialogStore.lastOpened) {
        dialogStore.close(props.dialogId);
        dialogStore.closeLastOpened();
    }
};

const confirm = () => {
    dialogStore.callback();
    close();
};
