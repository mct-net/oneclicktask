...
import type { Tag, Task, User } from '@/lib/types/models';
import { utcDate } from '@/lib/utils/date';
import { handleError } from '@/lib/utils/error';
import { filterOverdueOrDueToday, sortByUrgency } from '@/lib/utils/tasks';
    ...
    */
    ...
    async function removeTag(task: Task, tagId: number) {
        try {
            ...
            uiStore.quickTagFilters = uiStore.quickTagFilters.filter(
                (tag) => tag.id !== tagId,
            );
        } catch (error) {
            handleError(error);
    ...
    async function removeComment(task: Task, commentId: number) {
        try {
            await restClient.comments.delete(commentId);
    ...
    async function updateComment(
        ...
        try {
            await restClient.comments.update(commentId, content);
